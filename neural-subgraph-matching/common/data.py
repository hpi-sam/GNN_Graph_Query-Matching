...
class LDBCDataSource(DataSource):
    ...
    def gen_neg_target(self):
        # TODO
        graphs = [nx.complete_graph(self.sample_size)
                  for i in range(self.num_graphs)]
        dataset = GraphDataset(graphs=graphs)
        return dataset

    def gen_neg_query(self):
        # TODO
        graphs = [nx.complete_graph(self.sample_size)
                  for i in range(self.num_graphs)]
        dataset = GraphDataset(graphs=graphs)
    ...
    def gen_data_loaders(self, size, batch_size, train=True,
                         use_distributed_sampling=False):
        pos_target_loader = TorchDataLoader(self.gen_pos_target(),
                                            collate_fn=Batch.collate([]), batch_size=batch_size // 2, shuffle=False)
        neg_target_loader = TorchDataLoader(self.gen_neg_target(),
                                            collate_fn=Batch.collate([]), batch_size=batch_size // 2, shuffle=False)
        neg_query_loader = TorchDataLoader(self.gen_neg_query(),
