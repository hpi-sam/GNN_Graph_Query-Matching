...
def peturb(A, choose, num_edges):
    G = nx.convert_matrix.from_numpy_matrix(A)
    centrality = nx.algorithms.centrality.katz_centrality(G)
    node = choose(centrality, key=centrality.__getitem__)
    G.remove_edges_from(list(nx.edges(G, node))[:num_edges])
...
class Net(CNN):
    ...
    def real_forward(self, data_dict, num_clusters, **kwargs):
        ...
        if 'images' in data_dict:
            ...
            As_src = torch.from_numpy(np.asarray([peturb(a[0].numpy(), min, 3) for a in As_src], dtype=np.float64)).float()
